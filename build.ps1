...
if ($startupgen)
{
    ...
    {
        Write-Host "Generation of $($linkerscript) FAILED"; exit 1 
    } else
    {
        if (!(Test-Path -Path $($crt0src)))
        { 
            Write-Host "Generation of $($crt0src) FAILED"; exit 1 
        }
    }
...
} else
{
    ...
    if (!(Test-Path -Path $($linkerscript)))
    {
        Write-Host "$($linkerscript) is missing"; exit 1
    } else
    {
        $crt0src = Join-Path $srcdir "crt0.s"
        if (!(Test-Path -Path $($crt0src)))
        {
            Write-Host "$($crt0src) is missing"; exit 1
        }        
    }
}
...
{
    Write-Host "Linking of $($image) FAILED";  exit 1 
}
